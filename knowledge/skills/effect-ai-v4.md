...
## Runtime model selector service

When model choice comes from runtime input, define a dedicated service that centralizes provider wiring, model capture, and per model override rules.
...
type ModelFamily = "sonnet" | "gpt-5";

const SonnetModel = AnthropicLanguageModel.model("claude-sonnet-4-5-20250929");
const Gpt5Model = OpenAiLanguageModel.model("gpt-5.2", {
...
export class AiModels extends Context.Service<AiModels, {
  readonly use: (
    model: ModelFamily,
    config?: { readonly temperature?: number; },
  ) => <A, E, R>(
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function*() {
      ...
      const gpt5Model = yield* Gpt5Model.captureRequirements;
      ...
      return {
        use: (model, config) => (self) => {
          if (config === undefined) {
            return Effect.provide(self, getModelLayer(model));
          }
          switch (model) {
            case "sonnet":
              return self.pipe(
    ...
    Layer.provide(AiProviderLive),
    Layer.orDie,
  );
}
```

This keeps callsites small and pipeable:
...
}).pipe(aiModels.use(selectedModel));
```

When another dependency needs a `Layer` instead of an effect wrapper, expose `layer(model)` too:

```ts
someProgram.pipe(HistoryCompressionModel.provide(aiModels.layer("sonnet")));
```
